...
함수에서 물체가 생성되면 시제품이라고 하는 다른 물체에 대한 링크가 생성된다. 각각의 프로토타입 물체는 ____proto____에서 발견되는 고유의 프로토타입을 가지고 있다. 이것을 프로토타입 체인이라고 한다. 물체는 조상 객체 특성과 방법을 보유하는 프로토타입 링크를 통해 그것의 프로토타입에 연결된다. 이 프로토타입 체인은 물체의 ____proto____ 안에 있는 속성을 찾을 때까지 각각의 조상 프로토타입을 검색하여 찾을 수 있도록 한다. 속성이 존재하지 않으면 정의되지 않은 것이 반환된다.
